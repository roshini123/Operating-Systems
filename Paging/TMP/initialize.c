...
//	kprintf("\nd: Initializing Demand Paging Process Entry Parameters\n");
	...
	pptr->vmemlist = NULL;
	for (i = 0; i < BS_NUM; i++) {
		pptr->bsmap[i].bs_private = 0;
		pptr->bsmap[i].bs_npages = 0;
		pptr->bsmap[i].bs_pid = -1;
		pptr->bsmap[i].bs_refCount = 0;
		pptr->bsmap[i].bs_sem = -1;
		pptr->bsmap[i].bs_status = UNMAPPED;
		pptr->bsmap[i].bs_vpno = -1;
	}
	//
